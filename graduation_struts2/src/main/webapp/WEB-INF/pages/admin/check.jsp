		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="agreeForm" class="form-horizontal">
					<div class="form-group">
						<input class="form-control" type="hidden"  name="tid" underline="true"/>
...
var operation={
	...
	agreeWindow:function(){
		...
		if(rows.length == 1  ){
			...
			if(rows[0].ifPass == "未审核"){
				document.getElementsByName("opinion")[0].value = rows[0].opinion;
	...
	agreeAgain:function(){
			...
			$.post("adminAgreeModify.action", temp ,function(data) {
		//	var obj = eval('('+data+')');
	...
	disagreeWindow:function(){
		...
		if(rows.length == 1  ){
			...
			if(rows[0].ifPass == "未审核"){
				document.getElementsByName("opinion")[1].value = rows[0].opinion;
	...
	disagreeAgain:function(){
		...
		$.post("adminDisagreeModify.action", temp ,function(data) {
	//	var obj = eval('('+data+')');
		$("#dg").bootstrapTable("refresh");
		message.alert(data.showInfo);
...
var columns=[[
	...
	},  
	{
		...
		visible:false
	     ...
	     {title: '审核状态', field: 'ifPass', align: 'center', valign: 'middle',visible:true},
