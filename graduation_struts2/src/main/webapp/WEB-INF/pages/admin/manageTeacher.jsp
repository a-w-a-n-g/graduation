		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="setForm" class="form-horizontal">
					...
					<div class="form-group">
						<label class="col-sm-4 control-label"  for="id">编号</label>
						<div class="col-sm-6">
							    <input class="form-control" type="text"  name="teacher.id" underline="true"  placeholder="请输入编号"/>
					...
					<div class="form-group">
						...
						<div class="col-sm-6">
							   <input class="form-control" type="text"  name="teacher.name" underline="true" placeholder="请输入姓名"/>
							<!-- <input class="form-control" type="text" name = "roomName" placeholder="请输入教室名称"> -->
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-4 control-label" for="title">职称</label>
						<div class="col-sm-6">
							   <input class="form-control" type="text"  name="teacher.title" underline="true" placeholder="请输入职称"/>
							<!-- <input class="form-control" type="text" name = "roomName" placeholder="请输入教室名称"> -->
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-6">
							   <input class="form-control" type="text"  name="sex" underline="true" placeholder="请输入性别（男、女）"/>
							<!-- <input class="form-control" type="text" name = "roomName" placeholder="请输入教室名称"> -->
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-6">
							   <input class="form-control" type="text"  name="teacher.phone" underline="true"/>
							<!-- <input class="form-control" type="text" name = "roomName" placeholder="请输入教室名称"> -->
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-6">
							   <input class="form-control" type="text"  name="teacher.mail" underline="true"/>
							<!-- <input class="form-control" type="text" name = "roomName" placeholder="请输入教室名称"> -->
						</div>
					</div>
...
var operation={
	...
	addForm:function(){
		...
		if($("#setForm").data("bootstrapValidator").isValid()){
			var temp = getFormJson("setForm");
			 ...
			 $.ajax
	         ({
	                 url: 'insertTeacher.action', //用于文件上传的服务器端请求地址
	...
	editWindow:function(){
		...
		if(rows.length == 1  ){
			...
			document.getElementsByName('teacher.id')[0].value=rows[0].id;
			if(rows[0].name != undefined)
			document.getElementsByName('teacher.name')[0].value=rows[0].name;
			if(rows[0].phone != undefined)
			document.getElementsByName('teacher.title')[0].value=rows[0].title;
			if(rows[0].mail != undefined)
			document.getElementsByName('teacher.phone')[0].value=rows[0].phone;
			if(rows[0].mail != undefined)
			document.getElementsByName('teacher.mail')[0].value=rows[0].mail;
			$('#setModal').modal('show');
		...
		if(rows.length == 1  ){
			bootbox.confirm({
				...
				callback:function(result){
					if(result){
						$.post("deleteTeacher.action", {'teacher.id':rows[0].id} ,function(data) {
							$("#dg").bootstrapTable("refresh");
							message.alert(data.showInfo);
...
var columns=[[
	...
	},  
	{
		...
		visible:false
	     ...
	     {title: '教师编号', field: 'id', align: 'center', valign: 'middle',visible:true},
	     {title: '教师名字', field: 'name', align: 'center', valign: 'middle',visible:true},
	     {title: '教师职称', field: 'title', align: 'center', valign: 'middle',visible:true},
