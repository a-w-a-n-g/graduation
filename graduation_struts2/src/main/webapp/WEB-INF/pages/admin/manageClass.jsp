		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="setForm" class="form-horizontal">
					<div class="form-group">
						<div class="col-sm-6">
							    <input class="form-control" type="hidden"  name="classNum.id" underline="true"/>
					...
					<div class="form-group">
						<label class="col-sm-4 control-label" for="name">班级名称</label>
						<div class="col-sm-6">
							    <input class="form-control" type="text"  name="classNum.name" underline="true"  placeholder="请输入专业"/>
							<!-- <input class="form-control" type="text" name = "roomName" placeholder="请输入教室名称"> -->
						</div>
					</div>				
				</form>	
...
var operation={
	...
	addForm:function(){
		...
		if($("#setForm").data("bootstrapValidator").isValid()){
			var temp = getFormJson("setForm");
			 ...
			 $.ajax
	         ({
	                 url: 'insertClass.action', //用于文件上传的服务器端请求地址
	...
	editWindow:function(){
		...
		if(rows.length == 1  ){
			...
			document.getElementsByName('classNum.id')[0].value=rows[0].id;
			if(rows[0].name != undefined)
			document.getElementsByName('classNum.name')[0].value=rows[0].name;
			$('#setModal').modal('show');
		...
		if(rows.length == 1  ){
			bootbox.confirm({
				...
				callback:function(result){
					if(result){
						$.post("deleteClass.action", {'classNum.id':rows[0].id} ,function(data) {
							$("#dg").bootstrapTable("refresh");
							message.alert(data.showInfo);
...
var columns=[[
	...
	},  
	{
		...
		visible:false
	     ...
	     {title: '班级名称', field: 'name', align: 'center', valign: 'middle',visible:true},
