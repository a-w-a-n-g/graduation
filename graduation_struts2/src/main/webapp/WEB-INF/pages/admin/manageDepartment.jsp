		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="setForm" class="form-horizontal">
					...
					<div class="form-group">
						<label class="col-sm-4 control-label"  for="name">学院名</label>
						<div class="col-sm-6">
							    <input class="form-control" type="text"  name="department.name" underline="true"  placeholder="请输入学号"/>
						</div>
					</div>
				</form>	
			</div>
...
var operation={
	...
	addForm:function(){
		...
		if($("#setForm").data("bootstrapValidator").isValid()){
			var temp = getFormJson("setForm");
			 ...
			 $.ajax
	         ({
	                 url: 'insertDpm.action', //用于文件上传的服务器端请求地址
	...
	editWindow:function(){
		...
		if(rows.length == 1  ){
			...
			document.getElementsByName('department.id')[0].value=rows[0].id;
			document.getElementsByName('department.name')[0].value=rows[0].name;
			$('#setModal').modal('show');
		...
		if(rows.length == 1  ){
			bootbox.confirm({
				...
				callback:function(result){
					if(result){
						$.post("deleteDpm.action", {'department.id':rows[0].id} ,function(data) {
							$("#dg").bootstrapTable("refresh");
							message.alert(data.showInfo);
...
var columns=[[
	...
	},  
	{
		...
		visible:false
	     ...
	     {title: '学院名字', field: 'name', align: 'center', valign: 'middle',visible:true},
